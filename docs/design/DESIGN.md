...
| 요소 | 시각 처리 | 의미 |
|---|---|---|
| **Todogram task** (내 것) | 순백 배경, 좌측 3px 실선 보더(해당 라벨 색), 1px 보더 전체, soft shadow | "위에 앉아있다 — 내가 만들고 내가 처리한다" |
...
#### Context-scoped 해석 — Today View row (2026-04-17 추가)

Today View 의 스크롤 스트림 맥락에서는 카드 형태 대신 **고정 높이 row 형태**를 사용합니다. 이때 "유리 뒤" 은유를 다음과 같이 재해석합니다 (approved: `~/.gstack/projects/SHIN-HANBEEN-Todogram3/designs/today-view-20260417/approved.json`):

- 내 태스크와 외부 이벤트 모두 **동일한 row geometry** ( `[time 56px | check 18px | title flex | chip auto]`, min-height 56px).
- 모든 row 의 **좌측 3px 보더 색 = 해당 row 의 라벨 색**을 계승 (내부/외부 구분 없음).
- 외부 이벤트는 reserved 라벨 `calendar` (dust-blue) 로 매핑 → 자동으로 dust-blue 좌측 틱.
- 내부/외부 구분 신호는 두 가지로 축소: (a) 체크박스 `visibility` (내부 보임 / 외부 hidden), (b) 제목 텍스트 색 톤 (내부 `text-primary` / 외부 `text-secondary`).
- dashed border · opacity 0.88 · bg-muted 는 row 형태에서는 **제거**. 카드 형태(주간 뷰 · 상세 시트 등)의 `ExternalEventCard` 는 기존 스타일 유지.
...
| Dust Blue | `#5A7A99` | `#87A4BE` | 9 (Blueberry) | **캘린더 (reserved, 외부 이벤트 전용)** |
| Amber | `#C08A3E` | `#D9AC70` | 5 (Banana) | 학습 / 사이드 프로젝트 |
| Plum | `#8C5A6E` | `#B18597` | 3 (Grape) | 개인 / 건강 |
| Moss | `#7A8B4A` | `#A5B57E` | 2 (Sage) | 취미 / 예비 |

칩 스타일: 배경 = hex 12% 알파, 텍스트 = hex full. 다크모드 배경 = hex 20% 알파.

**Reserved 라벨 규칙** (2026-04-17 추가):
- `calendar` (dust-blue) — 외부 캘린더 이벤트 전용. 사용자가 임의로 할당/편집 불가.
- 사용자가 새 라벨을 만들 때 dust-blue 는 팔레트에서 제외됨(캘린더와의 충돌 방지). 기본 5색(sage/terracotta/amber/plum/moss)과 7번째 이후 추가 색상만 사용자 라벨로 선택 가능.
- v1 기본 사용자 라벨 프리셋: `직장 (sage) · 가정 (terracotta) · 학습 (amber) · 개인 (plum)` — 이전 매핑에서 `개인`을 dust-blue → plum 으로 재배정.
...
| Info | Dust Blue | Dust Blue label (캘린더 reserved 와 같은 색 — 시맨틱 맥락에서 충돌 없으면 재사용 가능) |
...
|---|---|---|
| `TaskCard` | `src/components/todogram/task-card.tsx` | 내 태스크 카드 (주간 뷰·상세 시트용) — 좌측 라벨 색 3px 보더, shadow-card, 체크박스, 라벨 칩 |
| `ExternalEventCard` | `src/components/todogram/external-event-card.tsx` | 외부 캘린더 이벤트 카드 (주간 뷰·상세 시트용) — dashed 보더, opacity 0.85, 시계 아이콘 |
| `LabelChip` | `src/components/todogram/label-chip.tsx` | 6색 라벨 칩 — 배경 12% 알파 + 텍스트 full. dot variant 도 지원 |
| `TodayHeader` | `src/components/todogram/today-header.tsx` | Instrument Serif로 렌더한 날짜 + 오늘/내일/이번 주 토글 |
| `BottomNav` | `src/components/todogram/bottom-nav.tsx` | 4탭 하단 네비 (Today / Calendar / Labels / Settings). active underline `bottom: 8px` |
| `Fab` | `src/components/todogram/fab.tsx` | 플로팅 + 버튼, 하단 네비 위 (56px · sage solid · shadow-float · `md:hidden`) |
| `TodayView` | `src/components/todogram/today-view.tsx` | Today View 스크린 컨테이너 — filter rail + stream + banner orchestration |
| `TodayRow` | `src/components/todogram/today-row.tsx` | Today View 통합 row — 내부/외부 동일 geometry, 좌측 보더 색 = 라벨 색 계승 |
| `FilterRail` | `src/components/todogram/filter-rail.tsx` | Sticky 라벨 필터 바 — LabelChip outline + scroll-x |
| `RolloverBanner` | `src/components/todogram/rollover-banner.tsx` | 이월 건 요약 배너 (amber tint + 유지/보관 액션) |
| `LoginForm` | `src/components/login-form.tsx` | Quiet Lamp 레이아웃 — 카드 chrome 없음, Instrument Serif 워드마크 + sage Google + 검정 Apple + 이메일 disclosure. OAuth 우선 흐름. |
...
| 2026-04-17 | `Fab` — **Variant A "Sage Solid (Canonical)"** 채택 | `/design-shotgun` 세션. 4개 변형(A Sage Solid · B Quiet Outline · C Extended Pill · D Layered Plate) HTML 프리뷰 비교. A 선택 이유: (1) 발견성 9/10 — 아침 2초 체크 정체성에 부합, (2) sage solid 가 TaskCard 3px 좌측 보더 · BottomNav sage underline 과 함께 sage 삼위일체(fill ↔ line ↔ underline) 완성, (3) DESIGN.md §5·§6·§7 정공법. 세부 스펙: `~/.gstack/projects/SHIN-HANBEEN-Todogram3/designs/fab-variants-20260417/approved.json`. |
| 2026-04-17 | Today View — **Variant D refined v3 "Unified Ledger"** 채택 | `/design-shotgun` 세션 3 iteration. 4 variant 비교 후 D 선택 → row geometry 통일(내부/외부 동일, min-height 56px, `[time 56px · check 18px · title flex · chip auto]`) + LabelChip dot variant → filter-color 계승 (모든 row `border-left: 3px` = 라벨 색) + dust-blue 를 `calendar` reserved 로 재할당, `개인` 기본 라벨을 dust-blue → plum 재배정 → mid-gap divider element (`flex: 0 0 1px`, row 사이 중앙에 위치) + BottomNav active underline `bottom` 14px → 8px 조정. 세부 스펙: `~/.gstack/projects/SHIN-HANBEEN-Todogram3/designs/today-view-20260417/approved.json`. |
| 2026-04-18 | Login — **Variant A "Quiet Lamp"** 채택 | `/design-shotgun` 세션. 3 variant (A Quiet Lamp · B Canvas · C Ledger) HTML 프리뷰 비교 후 A 선택. 선택 이유: (1) 가장 Quiet — 카드 chrome · 그라디언트 · 컨페티 0개 (§9 Hard Rules 완전 준수), (2) 인지 속도 — 상단 램프 아이콘 + Instrument Serif 워드마크 + 인사말로 방문자가 "맞게 왔다"를 0.5초 안에 인지, (3) OAuth 우선 흐름 — Google(brand-solid sage) + Apple(black-solid) 2단 스택으로 V1 본인 사용 시나리오 최적화, 이메일은 `ㆍ이메일로 계속 ▾` disclosure 로 접힘. (4) sage 삼위일체 완성 — TaskCard 좌측 3px border · BottomNav underline · FAB solid · Login Google primary 네 축이 모두 sage solid 로 통합. 세부 스펙: `~/.gstack/projects/SHIN-HANBEEN-Todogram3/designs/login-variants-20260418/approved.json`. |
...
4. **Todogram 핵심 컴포넌트 구현** (§8) — `TaskCard` · `ExternalEventCard` · `LabelChip` · `TodayHeader` · `BottomNav` · `Fab` (완료) + Today View 전용 4종 (`TodayView` · `TodayRow` · `FilterRail` · `RolloverBanner`)
5. **Today View 스크린 구현** — 스펙 확정됨 (`~/.gstack/projects/SHIN-HANBEEN-Todogram3/designs/today-view-20260417/approved.json`). 카드 대신 통합 row 레저(ledger) 레이아웃, filter rail + stream + rollover banner 조합.
6. **라벨 상수 파일 구축** — `calendar` reserved 라벨 매핑 + 기본 사용자 라벨 4종 (직장/가정/학습/개인) 프리셋 정의.
7. **로그인 · 회원가입 화면** — `src/components/login-form.tsx` Quiet Lamp 재작성 완료 (2026-04-18). NextAuth v5 연결(F3)과 함께 OAuth onClick 을 `signIn('google' | 'apple')` 로 교체. 회원가입 화면도 동일 톤으로 별도 재작성 예정.
8. **실기기 테스트** — 본인 Android + Samsung 캘린더 위에서 dogfooding
