...
export const PlayButtonIcon = ({
  isPlaying = false,
  ...props
}: HTMLAttributes<HTMLDivElement> & { isPlaying?: boolean }) => {
  return (
    <div
      {...props}
      className={cx(
        'bg-alpha-white/30 group-hover:bg-alpha-white/50 hover:bg-alpha-white/50 flex size-20 items-center justify-center rounded-full backdrop-blur transition duration-100 ease-linear',
        props.className
      )}
    >
      {isPlaying ? (
