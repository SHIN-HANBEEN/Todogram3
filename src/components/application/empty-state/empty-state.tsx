...
const FeaturedIcon = ({
  ...
  return (
    <FeaturedIconbase
      {...props}
      {...{ color, theme, icon }}
      size={!size && rootSize === 'lg' ? 'xl' : size || 'lg'}
    />
  )
}
...
const FileTypeIcon = ({
  ...
  return (
    <div
      {...props}
      className={cx(
        'relative z-10 flex rounded-full bg-linear-to-b from-neutral-50 to-neutral-200 p-8',
        props.className
      )}
    >
      <FileIcon
...
const AvatarRadius = ({ avatars = [], ...props }: AvatarRadiusProps) => {
  return (
    <div
      ...
      {...props}
      className={cx(
        'pointer-events-none absolute top-1/2 left-1/2 size-120 -translate-x-1/2 -translate-y-1/2',
        props.className
      )}
      style={{
    ...
    >
      {RING_RADII.map(radius => (
        <div
          key={radius}
          className="border-secondary absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full border"
      ...
      {avatars.slice(0, avatarSlots.length).map((avatar, i) => {
        ...
        return (
          <div
            key={i}
            className={cx(
              'bg-primary absolute top-1/2 left-1/2 rounded-full p-px shadow-xs ring-[0.5px] ring-black/10',
              slot.size
            )}
            style={{
  ...
  return (
    <div
      ...
      className={cx(
        ...
        sizeKey === 'sm' ? 'gap-4' : sizeKey === 'md' ? 'gap-5' : 'gap-6',
        props.className
      )}
      style={{
    ...
    >
      ...
      >
        {rightAvatars.map((avatar, i) =>
          renderAvatar(avatar, sizes[count - 1 - i], i + count)
        )}
      </div>
    </div>
  )
...
const AvatarGrid = ({ avatars = [], ...props }: AvatarGridProps) => {
  ...
  const renderGridAvatar = (
    ...
    <div
      key={key}
      className={cx(
        'bg-primary shrink-0 p-px shadow-xs ring-[0.75px] ring-black/10',
        config.avatar
      )}
    >
      <div
  ...
  return (
    ...
    >
      <div className="flex">
        <div
          className={cx(
            'animate-marquee flex w-auto max-w-none shrink-0 [animation-duration:240s] motion-reduce:animate-none',
            config.gap
          )}
        >
      ...
      <div className="flex">
        <div
          className={cx(
            'animate-marquee direction-reverse flex w-auto max-w-none shrink-0 [animation-delay:-120s] [animation-duration:240s] motion-reduce:-translate-x-1/2 motion-reduce:animate-none',
            config.gap
          )}
        >
