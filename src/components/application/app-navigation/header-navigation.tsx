    ...
    <>
      <div className="flex gap-0.5">
        ...
        <div className="relative">
          ...
          <div className="bg-fg-error-primary absolute -top-0.25 -right-0.25 flex size-3.5 items-center justify-center rounded-full text-[10px] font-bold text-white">
            2
          </div>
        </div>
      </div>
...
export const HeaderNavigationBase = ({
  ...
  const isActive = (item: NavItem) =>
    item.current ?? isItemActive(item.href, activeUrl)

  const activeParent = items.find(
    item =>
      isActive(item) ||
      item.items?.some(sub => isItemActive(sub.href, activeUrl))
  )
  ...
  return (
    <>
      ...
      <header className="max-lg:hidden">
        <section
          className={cx(
            'bg-primary flex h-16 w-full items-center justify-center',
            (!hideBorder || showSecondaryNav) && 'border-secondary border-b'
          )}
        >
          <div
            className={cx(
              'max-w-container flex w-full items-center pr-3 pl-4 md:px-8',
              centered && 'gap-8'
            )}
          >
            <div
              className={cx('flex items-center', centered ? 'flex-1' : 'mr-4')}
            >
            ...
            <div
              className={cx(
                ...
                centered ? 'flex-1 justify-end' : 'ml-auto'
              )}
            >
              {hasCustomActions ? (
                actions
              ) : (
                <DefaultActions activeUrl={activeUrl} />
              )}
            </div>
          </div>
        </section>
        ...
        {showSecondaryNav && (
          <section
            className={cx(
              'bg-primary flex w-full items-center justify-center',
          ...
          >
            {secondaryType === 'tabs' ? (
              ...
              <div
                className={cx(
                  'max-w-container flex h-16 w-full items-center gap-8 px-8',
