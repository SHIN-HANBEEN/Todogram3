          ...
          <div className="flex flex-col gap-0.5 px-1.5">
            {placeholderAccounts.map(account => (
              <button
                key={account.id}
                className={cx(
                  'outline-focus-ring hover:bg-primary_hover relative w-full cursor-pointer rounded-md px-2 py-1.5 text-left transition duration-100 ease-linear focus:z-10 focus-visible:outline-2 focus-visible:outline-offset-2',
    ...
    >
      ...
      >
        <div className="text-secondary group-hover/item:text-secondary_hover flex gap-2 text-sm font-semibold">
          {Icon && (
            <Icon className="text-fg-quaternary group-hover/item:text-fg-quaternary_hover size-5" />
          )}{' '}
          {label}
        </div>
...
export const NavAccountCard = ({
  ...
  return (
    ...
    >
      ...
      <AriaDialogTrigger>
        <AriaButton className="text-fg-quaternary outline-focus-ring hover:bg-primary_hover hover:text-fg-quaternary_hover pressed:bg-primary_hover pressed:text-fg-quaternary_hover absolute top-2 right-2 flex cursor-pointer items-center justify-center rounded-md p-1.5 transition duration-100 ease-linear focus-visible:outline-2 focus-visible:outline-offset-2">
