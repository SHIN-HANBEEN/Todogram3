...
export const Dialog = (props: DialogProps) => (
  <AriaDialog
    ...
    {...props}
    className={cx(
      'bg-primary ring-secondary_alt relative flex size-full flex-col items-start gap-6 overflow-y-auto ring-1 outline-hidden',
      props.className
    )}
  />
)
...
const Footer = (props: ComponentPropsWithRef<'footer'>) => {
  return (
    <footer
      {...props}
      className={cx(
        'shadow-border-secondary w-full p-4 shadow-[inset_0px_1px_0px_0px] md:px-6',
        props.className
      )}
    />
  )
