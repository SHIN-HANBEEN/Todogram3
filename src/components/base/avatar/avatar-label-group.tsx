...
export const AvatarLabelGroup = ({
  ...
  return (
    <figure
      className={cx('group flex min-w-0 flex-1 items-center gap-2', className)}
    >
      <Avatar border rounded={rounded} className={avatarClassName} {...props} />
      <figcaption className="min-w-0 flex-1">
        <p
          className={cx('text-primary font-semibold', styles[props.size].title)}
        >
          {title}
        </p>
        <p
          className={cx('text-tertiary truncate', styles[props.size].subtitle)}
        >
          {subtitle}
        </p>
      </figcaption>
