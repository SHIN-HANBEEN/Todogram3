...
import {
  type SelectItemType,
  sizes,
} from '@/components/base/select/select-shared'
...
const InnerTagSelect = ({
  ...
  placeholder,
  size = 'sm',
}: Omit<TagSelectProps, 'selectedItems' | 'children'>) => {
    ...
    <div className="relative flex w-full min-w-0 flex-1 flex-row flex-wrap items-center justify-start gap-1.5">
      ...
      >
        <AriaInput
          ...
          className={cx(
            'text-primary caret-alpha-black/90 placeholder:text-placeholder w-full flex-[1_0_0] appearance-none bg-transparent text-ellipsis outline-hidden focus:outline-hidden disabled:cursor-not-allowed',
            sizes[size].text
          )}
        />
  ...
  return (
    <AriaGroup
      ...
      className={({ isFocusWithin, isDisabled }) =>
        cx(
          'bg-primary ring-primary relative flex w-full items-center rounded-lg shadow-xs ring-1 outline-hidden transition duration-100 ease-linear ring-inset',
