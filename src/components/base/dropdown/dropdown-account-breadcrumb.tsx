...
export const DropdownAccountBreadcrumb = () => {
  const [selectedAccountKey, setSelectedAccountKey] =
    useState<string>('caitlyn')

  const selectedAccount = accounts.find(
    account => account.id === selectedAccountKey
  )
      ...
      <Dropdown.Popover className="w-62" placement="bottom left">
        <Dropdown.Menu
          ...
          onSelectionChange={keys =>
            setSelectedAccountKey(Array.from(keys).join())
          }
        ...
        >
          {accounts.map(account => (
            <AriaMenuItem
              ...
              className={state =>
                cx(
                  'outline-focus-ring hover:bg-primary_hover relative w-full cursor-pointer rounded-md px-2 py-2 text-left transition duration-100 ease-linear focus:z-10 focus-visible:outline-2 focus-visible:outline-offset-2',
