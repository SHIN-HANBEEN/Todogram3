...
### 3. UntitledUI React 활용 (UI 구성 단계 - 권장)

`npx untitledui@latest add` CLI를 사용하여 페이지 구조 생성 시 필요한 UI 컴포넌트를 즉시 설치합니다.
...
```bash
# 필요한 컴포넌트 추가
npx untitledui@latest add skeleton
npx untitledui@latest add button alert
npx untitledui@latest add navigation-menu breadcrumb
npx untitledui@latest add card button
```

```typescript
// Context7 MCP로 UntitledUI 컴포넌트 문서 조회
// resolve-library-id("untitled-ui")
// query-docs(topic: "skeleton component")
```
...
Phase 3: 구조 생성 (파일/폴더)
   ↓
Phase 4: UI 컴포넌트 준비 (UntitledUI React)
   ↓
Phase 5: 코드 작성
   ↓
...
2. **컴포넌트 확인**
   ...
   // Context7 MCP로 UntitledUI 컴포넌트 문서 조회
   // resolve-library-id("untitled-ui")
   // query-docs(topic: "skeleton button alert")
   ```

3. **설치 명령 실행**
   ```bash
   npx untitledui@latest add skeleton button alert navigation-menu breadcrumb
   ```
...
#### Step 4: UntitledUI React 컴포넌트 설치
...
```typescript
// Context7 MCP로 컴포넌트 사용법 확인 (선택)
// resolve-library-id("untitled-ui")
// query-docs(topic: "navigation menu breadcrumb")
```
