...
### 3. UntitledUI React 컴포넌트 참조

**사용 시기:**

- 프로젝트에 추가할 UntitledUI 컴포넌트를 찾을 때
- 컴포넌트 사용 예제를 참조할 때
- 컴포넌트의 정확한 props와 구조를 확인할 때

**참조 방법:**

1. **Context7 MCP로 UntitledUI 문서 조회**

   ```
   resolve-library-id("untitledui") 또는 "untitled-ui"
   query-docs로 특정 컴포넌트 패턴 확인
   ```
...
- Context7 MCP로 UntitledUI 컴포넌트 및 최신 문서 참조
- 프로젝트 가이드 문서 확인
...
- [ ] 한국어 주석이 마크업 구조를 설명함
- [ ] 기능적 로직이 구현되지 않음
- [ ] UntitledUI React 컴포넌트가 적절히 통합됨
- [ ] UntitledUI 테마를 따름
...
Stage 2: Information Gathering
- Context7 MCP로 UntitledUI Card 컴포넌트 검색
- 유사한 예제 확인
...
2. **Context7 MCP로 UntitledUI 컴포넌트 패턴 확인**

```
resolve-library-id("untitled-ui") 또는 "react-aria"
query-docs(
  topic: "card component patterns"
)
```
...
2. **Context7로 UntitledUI Table 패턴 확인**

```
resolve-library-id("untitled-ui")
query-docs(topic: "table responsive")
```
...
- **예제를 참조하세요**: Context7 MCP로 UntitledUI 실제 구현 예제를 찾으세요
